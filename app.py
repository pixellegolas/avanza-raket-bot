...
BUDGET=10000
last_scan={"budget":BUDGET,"max_daily_loss":500,"portfolio":{"total_pnl_after":0,"win_rate_after":0,"daily_pnl":0,"wins_after":0,"total":0,"trades":[]},"raketer":[],"watchlist_prices":[],"status":"Init","time":datetime.now().isoformat(),"news":[]}
...
def get_data(t):
    try:
        for period in ["6mo","3mo","1mo"]:
            ...
            close=df['Close']; price=float(close.iloc[-1]); prev=float(close.iloc[-2])
            ...
            return {"price":price,"change":change,"vol_ratio":vol_ratio,"rsi":rsi_val,"sma20":sma20,"sma50":sma50}
    except: pass
    return None
...
def job(force=False):
    ...
    for item in WATCHLIST:
        ...
        if not d: watch.append({**item,"price":0,"change":0,"score":0,"reasons":["Ingen data"]}); continue
        sc,rs=score(d)
        watch.append({**item,"price":d["price"],"change":d["change"],"score":sc,"reasons":rs})
    ...
    last_scan["status"]=f"FINAL 3 raketer - {now.strftime('%H:%M:%S')} - {len([w for w in watch if w['price']>0])}/{len(WATCHLIST)} priser OK"
